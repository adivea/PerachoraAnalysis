...
# OSM Greek data
places <- st_read("data/Greece/places.shp")
...
plot(elev, main = "2020 PPAP study area"); plot(bbb$geometry, add = TRUE); plot(box$geometry, border = "red", add =TRUE)
...
```
```{r results-features}
tm_shape(units, bbox = st_bbox(units), unit = "m") + 
  tm_polygons(col="cornsilk2", border.alpha = 0)+
  tm_shape(features) + 
  tm_symbols(shape = 6, col="#666666", size = 0.2) +
  tm_shape(border_ppap) +
...
summary(units$elev)
hist(el_ssm); hist(units$elev, t , add = TRUE)

SU_elev <- zonal(elev, rasterize(units, elev, class = "SUID"), fun = "max")
SU_elev
```
